...
getCambicBounds <- function(p,
  ...
  if (is.null(argi_bounds)) {
    argi_bounds <- sharpshootR::getArgillicBounds(p, hzdesgn, clay.attr, texcl.attr, ...)
  }
  ...
  for (j in 1:nrow(nhz)) {
    for (i in j:nrow(nhz)) {
      ...
      if (any(hzDepthTests(data.frame(ftop, fbot)))) {
        i <- i - 1
        break;
      }
    }

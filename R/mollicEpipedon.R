...
mollic.thickness.requirement <- function(p, texcl.attr = guessHzTexClName(p),
  ...
  #   but technically it is not applying the true taxonomic rules b/c it is based on hz desgn
  natric_bottom <- maxDepthOf(p,
                              pattern = "n",
                              top = FALSE,
                              no.contact.assigned = NA)
  oxic_bottom <- maxDepthOf(p,
                            pattern = "o",
                            top = FALSE,
                            no.contact.assigned = NA)
  spodic_bottom <- maxDepthOf(p,
                              pattern = "h|s[^s]",#NB slickensides!!!!!
                              top = FALSE,
                              no.contact.assigned = NA)

  #   the B|w == cambic was particularly egregious
  cambic_bottom <- suppressWarnings(max(
      getCambicBounds(p, argi_bounds = argi_bounds, texcl.attr = texcl.attr, clay.attr = clay.attr)$cambic_bottom,
      na.rm = TRUE
    ))

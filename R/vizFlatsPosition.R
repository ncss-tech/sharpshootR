...
#' 
#' @note Default colors are from `PNWColors::pnw_palette('Shuksan2')`.
#' 
...
#' 
#' 
vizFlatsPosition <- function(x, s = NULL, annotations = TRUE, annotation.cex = 0.75, clust = TRUE, dend.size = 5, dend.type = c('rectangle', 'triangle'), cols = c("#5d74a5", "#b0cbe7", "#fef7c7", "#a8554e"), ...) {
  ...
  if(n.series > 1) {
    ...
    if(clust) {
      ...
      .res <- hydOrder(x, g = 'flats', clust = FALSE)

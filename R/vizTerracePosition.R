...
#' 
#' 
vizTerracePosition <- function(x, s = NULL, annotations = TRUE, annotation.cex = 0.75, clust = TRUE, dend.size = 5, dend.type = c('rectangle', 'triangle'), cols = c("#5d74a5", "#fef7c7"), ...) {
  ...
  if(n.series > 1) {
    ...
    if(clust) {
      ...
      .res <- hydOrder(x, g = 'terrace', clust = FALSE)
